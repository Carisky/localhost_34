...
class __TwigTemplate_ad8d246acac7ff55ec5cb63ef8ddb149 extends Template
{
    private $source;
    private $macros = [];
...
\t\t\t                // Отображаем сообщение с номером заказа
\t\t\t                nrShowMessage(json.message, 1);
\t\t\t                // Очищаем список товаров (например, обнуляем содержимое блока с товарами)
\t\t\t                \$('.wholesale-products').html('');
\t\t\t                // Если требуется, сбрасываем форму
\t\t\t                \$('#wholesale')[0].reset();
\t\t\t            }
\t\t\t        },
\t\t\t        error: function(data) {
