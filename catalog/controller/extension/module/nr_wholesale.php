...
class ControllerExtensionModuleNrWholesale extends Controller {
...

	{
		...
		if(false) {
			...
			foreach ($categories as &$category) {
				...
				$products = $results = $this->model_catalog_product->getProducts([
			...
			if(!empty($this->session->data['wholesale_address'])) {
				...
				$data['address'] = $this->session->data['wholesale_address'];
				...
				$data['address'] = [
					...
					//'address_1' => $this->customer->getCustomFields()[3],
...

	{
		...
		if(!$phone and !empty($this->session->data['wholesale_address']['telephone'])) $phone = $this->session->data['wholesale_address']['telephone'];

		if(!$phone) $phone = $this->customer->getTelephone();
		...
		foreach($data_address as $key => $value) {
			...
			if($value) $this->session->data['wholesale_address'][$key] = $value;
	...
	{ //echo "<pre>"; print_r($this->request->post); die();
		...
		if($error) $this->sendJSON(['error' => $error]);
...

	{
		...
		if ((utf8_strlen(trim($address['firstname'])) < 1) or (utf8_strlen(trim($address['firstname'])) > 32)) $error['firstname'] = $this->language->get('error_firstname');

		if ((utf8_strlen(trim($address['lastname'])) < 1) or (utf8_strlen(trim($address['lastname'])) > 32)) $error['lastname'] = $this->language->get('error_lastname');

		if ((utf8_strlen($address['email']) > 96) or !filter_var($address['email'], FILTER_VALIDATE_EMAIL)) $error['email'] = $this->language->get('error_email');

		if ((utf8_strlen($address['telephone']) < 7) or (utf8_strlen($address['telephone']) > 20)) $error['telephone'] = $this->language->get('error_telephone');

		if (!trim($address['address_1'])) $error['address_1'] = $this->language->get('error_shipping_data');
