 ...
 */
$options['baselinker_pass'] = 'uqqzzhrgiwvchfevmuh8rrmvp8j4iz49';		// communication password, provided by Baselinker on the connection form

$options['db_host'] = 'localhost';		// database host (commonly localhost)
$options['db_user'] = 'userthejar';		// database user
$options['db_pass'] = 'kfWqQas3a8wVtfB';		// database password
$options['db_name'] = 'dbthejar';		// database name
$options['db_prefix'] = 'jar_';			// database table prefix, e.g. oc_, leave empty to detect automatically

$options['images_folder'] = 'https://thejar.com.pl/image/';  // root URL for the images folder (end with a single slash)
...
function Shop_FileVersion($request)
{
	...
	$response['version'] = "4.2.4"; //wersja pliku integracyjnego, nie wersja sklepu!
...
function Shop_ProductsData($request)
{
	...
	$sql = "SELECT DISTINCT p.product_id id, pd.name, pd.description description,
		...
		if(NOT isnull(s.price) AND {2} AND s.date_start <= '" . date('Y-m-d') . "' AND (s.date_end >= '" . date('Y-m-d') . "' OR s.date_end = '0000-00-00'), s.price, p.price)*${options['currency_mult']} price
		FROM `${dbp}product` p 
...
function Shop_OrdersGet($request)
{
	...
	while($order = DB_Fetch($res))
	{
		...
		$o['delivery_company'] = $order['shipping_company'];
		...
		$o['invoice_company'] = $order['payment_company'];
		...
		elseif (preg_match('/payu|dotpay|tpay|pp_standard|cashbill|eway|przelewy24/i', $order['payment_code']))
		{
		...
		while ($product = DB_Fetch($result))
		{
			...
			$op['tax'] = ($tax_rate > 0) ? $tax_rate : $options['def_tax'];
